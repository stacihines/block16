const items = [
  ...
  function capitalizeNames() {
     const mappedArray = items.map((grocery)=>{
      return grocery.name.toUpperCase();
  ...
  function calculateAllInventoryPrice() {
    return items.reduce((total, 
      item) => total + item.price * item.inventory, 0);
  ...
  function calculateTotalCartPrice() {
    ...
    return totalPrice.reduce((acc, price) => {
      acc = acc + price;
      return acc
    }, 0);
